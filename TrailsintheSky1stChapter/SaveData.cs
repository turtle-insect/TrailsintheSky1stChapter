...
namespace TrailsintheSky1stChapter
{
	internal class SaveData
	{
		...
		public bool Open(String filename, bool force, bool isSteam)
		{
			...
			if (!force)
			{
				var checksum = ReadNumber(0x08, 4);
		...
		private uint CalcCheckSum()
		{
			...
			uint checksum = 0x1F3FFC;
			for (uint i = 0; i < 0x7CFFF; i++)
			{
				for (uint j = 0; j < 4; j++)
				{
					uint address = i * 4 + 0x0C + j;
					Byte index = (Byte)(mBuffer[address] ^ (checksum & 0xFF));
					if (j == 1)
					{
						index = (Byte)(checksum & 0xFF);
						index ^= mBuffer[address];
					}
					checksum >>= 8;
					checksum ^= tables[index];
				}
			}
